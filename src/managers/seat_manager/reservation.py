    ...
    async def reserve_seat(self, username: str, seat_number: int) -> dict:
        ...
        try:
            ...
            if existing_reservation:
                self.handler.logger.info(f"Found existing reservation for user {username} on seat {existing_reservation.seat_number}, removing it")
            ...
            if existing_reservation:
                self.handler.logger.info(f"Seat {seat_number} is already reserved by {existing_reservation.user.username} (level {existing_reservation.user.level})")
                # If the seat is reserved, check if current user has higher level
                if user.level <= existing_reservation.user.level:
                    self.handler.logger.warning(f"User {username} (level {user.level}) cannot override reservation of {existing_reservation.user.username} (level {existing_reservation.user.level})")
                ...
                self.handler.logger.info(f"User {username} has higher level ({user.level} > {existing_reservation.user.level}), removing existing reservation")
                await SeatReservationDAO.remove_reservation(existing_reservation)
                self.handler.logger.info("Existing reservation removed successfully")
            ...
            self.handler.logger.info(f"Reservation created successfully: seat {seat_number} for {duration_hours} hours")
