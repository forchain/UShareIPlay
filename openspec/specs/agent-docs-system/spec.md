...
### Requirement: Repository SHALL include an Agent E2E operational contract
The repository SHALL include an `agent/` directory documenting the Agent E2E operational contract and constraints, including: readiness gates, event taxonomy, playbooks, known issues, and questions templates. The contract MUST encode the hard rule that external tools SHALL NOT create or connect a second Appium session for UI mutation.

#### Scenario: Agent enforces Appium session constraints
- **WHEN** an E2E test is planned
- **THEN** the agent SHALL only use console/queue for input and SHALL use read-only artifacts generated by the running process for UI evidence
